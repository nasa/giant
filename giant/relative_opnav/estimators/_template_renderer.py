...
class TemplateRenderer(UserOptionConfigured[TemplateRendererOptions], RelNavEstimator, TemplateRendererOptions, ABC):
    ...
    def compute_rays(self, target: SceneObject,
        ...
        if np.any(min_inds > max_inds):
            ...
            if self.template_overflow_bounds >= 0:
                min_inds = np.floor(min_inds).astype(int)
                max_inds = np.ceil(max_inds).astype(int)
                ...
                local_min = np.floor(local_min).astype(int)
                local_max = np.ceil(local_max).astype(int)
